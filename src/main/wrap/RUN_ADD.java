...
public class RUN_ADD extends Wrapper {
  ...
  protected void connectWires() {
    for (int i = 0; i < AdditionCommon.BIT_LENGTH; i++) {
      inputWire(Wrappers.X(i)).connectTo(subCircuits[0].inputWires, MOD_ADD_4N_N.A(i));
      inputWire(Wrappers.Y(i)).connectTo(subCircuits[0].inputWires, MOD_ADD_4N_N.B(i));
    }
  }
  ...
  public void fixConstants() {
    for (int i = 0; i < AdditionCommon.BIT_LENGTH; i++) {
      subCircuits[0].inputWire(MOD_ADD_4N_N.mMinusOne(i))
          .fixWire(AdditionCommon.MOD_MINUS_ONE.testBit(i) ? 1 : 0);
      subCircuits[0].inputWire(MOD_ADD_4N_N.invM(i))
