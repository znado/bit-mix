...
import circuit.mod.MOD_EXP_5N_N;
 ...
 */
public class RUN_EXP extends Wrapper {
  public static boolean wasRun = false;

  protected void createSubCircuits() throws Exception {
    subCircuits[0] = new MOD_EXP_5N_N(AdditionCommon.BIT_LENGTH);
  ...
  protected void connectWires() {
    for (int i = 0; i < AdditionCommon.BIT_LENGTH; i++) {
      inputWire(Wrappers.X(i)).connectTo(subCircuits[0].inputWires, MOD_EXP_5N_N.E(i));
    }
  }
  ...
  public void fixConstants() {
    for (int i = 0; i < AdditionCommon.BIT_LENGTH; i++) {
      subCircuits[0].inputWire(MOD_EXP_5N_N.mMinusOne(i))
          .fixWire(AdditionCommon.MOD_MINUS_ONE.testBit(i) ? 1 : 0);
      subCircuits[0].inputWire(MOD_EXP_5N_N.G(i))
          .fixWire(AdditionCommon.BASE.testBit(i) ? 1 : 0);
      subCircuits[0].inputWire(MOD_EXP_5N_N.invM(i))
